...
/*      */ 
/*      */       
/* 1032 */       int $i$a$-require-MutableLongFloatMap$1 = 0; String str = "Capacity must be a positive value."; throw new IllegalArgumentException(str.toString()); }  initializeStorage(ScatterMapKt.unloadedCapacity(initialCapacity));
/* 1033 */   } private final void initializeStorage(int initialCapacity) { int newCapacity = (initialCapacity > 0) ? Math.max(7, ScatterMapKt.normalizeCapacity(initialCapacity)) : 0; this._capacity = newCapacity; initializeMetadata(newCapacity); this.keys = new long[newCapacity]; this.values = new float[newCapacity]; } private final void initializeGrowth() { this.growthLimit = ScatterMapKt.loadedCapacity(getCapacity()) - this._size; } public final float getOrPut(long key, @NotNull Function0 defaultValue) { Intrinsics.checkNotNullParameter(defaultValue, "defaultValue"); int $i$f$getOrPut = 0, index = findKeyIndex(key); float defValue = ((Number)defaultValue.invoke()).floatValue(); put(key, defValue); return (index < 0) ? defValue : this.values[index]; } public final void set(long key, float value) { int index = findInsertIndex(key); if (index < 0) index ^= 0xFFFFFFFF;  this.keys[index] = key; this.values[index] = value; } private final void initializeMetadata(int capacity) { int size = (capacity + 1 + 7 + 7 & 0xFFFFFFF8) >> 3; long[] arrayOfLong2 = new long[size], arrayOfLong3 = arrayOfLong2; MutableLongFloatMap mutableLongFloatMap = this; int $i$a$-apply-MutableLongFloatMap$initializeMetadata$1 = 0; ArraysKt.fill$default(arrayOfLong3, -9187201950435737472L, 0, 0, 6, null); mutableLongFloatMap.metadata = (capacity == 0) ? ScatterMapKt.EmptyGroup : arrayOfLong2; long arrayOfLong1[] = this.metadata, value$iv = 255L; int $i$f$writeRawMetadata = 0; int i$iv = capacity >> 3;
...
/*      */   public final void put(long key, float value) { set(key, value); }
/* 1039 */   public final float put(long key, float value, float default) { int index = findInsertIndex(key); float previous = default; if (index < 0) { index ^= 0xFFFFFFFF; } else { previous = this.values[index]; }  this.keys[index] = key; this.values[index] = value; return previous; } public final void putAll(@NotNull LongFloatMap from) { Intrinsics.checkNotNullParameter(from, "from"); LongFloatMap this_$iv = from; int $i$f$forEach = 0; long[] k$iv = this_$iv.keys;
/* 1040 */     float[] v$iv = this_$iv.values;
/*      */     
/* 1042 */     LongFloatMap this_$iv$iv = this_$iv; int $i$f$forEachIndexed = 0;
...
/*      */             continue;  break; }  continue; if (i$iv$iv != lastIndex$iv$iv) { i$iv$iv++; continue; }  // Byte code: goto -> 220 }  } else {  }  } public final void plusAssign(@NotNull LongFloatMap from) { Intrinsics.checkNotNullParameter(from, "from"); int $i$f$plusAssign = 0; putAll(from); }
/*      */   public final void remove(long key) { int index = findKeyIndex(key); if (index >= 0)
/*      */       removeValueAt(index);  }
/*      */   public final boolean remove(long key, float value) { int index = findKeyIndex(key); if (index >= 0 && ((this.values[index] == value))) { removeValueAt(index); return true; }  return false; }
/* 1065 */   public final void removeIf(@NotNull Function2 predicate) { Intrinsics.checkNotNullParameter(predicate, "predicate"); int $i$f$removeIf = 0; LongFloatMap this_$iv = this; int $i$f$forEachIndexed = 0; long[] m$iv = this_$iv.metadata;
...
/* 1115 */       long key = content$iv[i$iv]; int $i$a$-forEach-MutableLongFloatMap$minusAssign$2 = 0; remove(key);
/*      */     }  } @PublishedApi public final void removeValueAt(int index) { this._size--; MutableLongFloatMap mutableLongFloatMap = this; long value$iv = 254L;
/*      */     int $i$f$writeMetadata = 0;
/* 1118 */     long[] m$iv = mutableLongFloatMap.metadata;
...
/*      */     
/* 1126 */     int c$iv = mutableLongFloatMap._capacity;
...
/* 1156 */       metadata$iv[index >> 3] >> (index & 0x7) << 3 & 0xFFL) == 128L)) ? 1 : 0; MutableLongFloatMap mutableLongFloatMap = this; long value$iv = hash2; int $i$f$writeMetadata = 0;
/* 1157 */     long[] m$iv = mutableLongFloatMap.metadata;
...
/*      */     
/* 1165 */     int c$iv = mutableLongFloatMap._capacity;
...
/* 1175 */   private final void resizeStorage(int newCapacity) { long[] previousMetadata = this.metadata; long[] previousKeys = this.keys; float[] previousValues = this.values; int previousCapacity = this._capacity; initializeStorage(newCapacity); long[] newKeys = this.keys; float[] newValues = this.values; for (int i = 0; i < previousCapacity; i++) { int $i$f$isFull = 0; int $i$f$readRawMetadata = 0; if (((
...
/* 1180 */         int $i$f$h1 = 0, index = findFirstAvailableSlot(hash >>> 7); MutableLongFloatMap mutableLongFloatMap = this; int $i$f$h2 = 0;
/* 1181 */         long value$iv = (hash & 0x7F); int $i$f$writeMetadata = 0;
/* 1182 */         long[] m$iv = mutableLongFloatMap.metadata;
...
/*      */         
/* 1190 */         int c$iv = mutableLongFloatMap._capacity;
...
/* Location:              C:\Users\yuzum\Downloads\ej-release-243.132.63.zip!\ej\lib\collection-jvm-1.4.0.jar!\androidx\collection\MutableLongFloatMap.class
