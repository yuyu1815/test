...
/*      */ 
/*      */       
/* 1059 */       int $i$a$-require-MutableObjectLongMap$1 = 0; String str = "Capacity must be a positive value."; throw new IllegalArgumentException(str.toString()); }  initializeStorage(ScatterMapKt.unloadedCapacity(initialCapacity));
/* 1060 */   } private final void initializeStorage(int initialCapacity) { int newCapacity = (initialCapacity > 0) ? Math.max(7, ScatterMapKt.normalizeCapacity(initialCapacity)) : 0; this._capacity = newCapacity; initializeMetadata(newCapacity); this.keys = new Object[newCapacity]; this.values = new long[newCapacity]; } private final void initializeMetadata(int capacity) { int size = (capacity + 1 + 7 + 7 & 0xFFFFFFF8) >> 3; long[] arrayOfLong2 = new long[size], arrayOfLong3 = arrayOfLong2; MutableObjectLongMap mutableObjectLongMap = this; int $i$a$-apply-MutableObjectLongMap$initializeMetadata$1 = 0; ArraysKt.fill$default(arrayOfLong3, -9187201950435737472L, 0, 0, 6, null); mutableObjectLongMap.metadata = (capacity == 0) ? ScatterMapKt.EmptyGroup : arrayOfLong2; long arrayOfLong1[] = this.metadata, value$iv = 255L; int $i$f$writeRawMetadata = 0; int i$iv = capacity >> 3;
/* 1061 */     int b$iv = (capacity & 0x7) << 3;
/*      */ 
/*      */     
/* 1064 */     arrayOfLong1[i$iv] = arrayOfLong1[i$iv] & (255L << b$iv ^ 0xFFFFFFFFFFFFFFFFL) | value$iv << b$iv; initializeGrowth(); } private final void initializeGrowth() { this.growthLimit = ScatterMapKt.loadedCapacity(getCapacity()) - this._size; } public final long getOrPut(Object key, @NotNull Function0 defaultValue) { Intrinsics.checkNotNullParameter(defaultValue, "defaultValue"); int $i$f$getOrPut = 0, index = findKeyIndex((K)key); if (index >= 0) return this.values[index];  long value = ((Number)defaultValue.invoke()).longValue(); set((K)key, value); return value; } public final void set(Object key, long value) { int index = findIndex((K)key); if (index < 0) index ^= 0xFFFFFFFF;  this.keys[index] = key; this.values[index] = value; } public final void put(Object key, long value) { set((K)key, value); }
/*      */   public final long put(Object key, long value, long default) { int index = findIndex((K)key); long previous = default; if (index < 0) { index ^= 0xFFFFFFFF; } else { previous = this.values[index]; }  this.keys[index] = key; this.values[index] = value; return previous; }
/* 1066 */   public final void putAll(@NotNull ObjectLongMap from) { Intrinsics.checkNotNullParameter(from, "from"); ObjectLongMap this_$iv = from; int $i$f$forEach = 0; Object[] k$iv = this_$iv.keys;
/* 1067 */     long[] v$iv = this_$iv.values;
/*      */     
/* 1069 */     ObjectLongMap this_$iv$iv = this_$iv; int $i$f$forEachIndexed = 0;
...
/*      */             continue;  break; }  continue; if (i$iv$iv != lastIndex$iv$iv) { i$iv$iv++; continue; }  // Byte code: goto -> 220 }  } else {  }  } public final void plusAssign(@NotNull ObjectLongMap<K> from) { Intrinsics.checkNotNullParameter(from, "from"); int $i$f$plusAssign = 0; putAll(from); }
/*      */   public final void remove(Object key) { int index = findKeyIndex((K)key); if (index >= 0)
/*      */       removeValueAt(index);  }
/*      */   public final boolean remove(Object key, long value) { int index = findKeyIndex((K)key); if (index >= 0 && this.values[index] == value) { removeValueAt(index); return true; }  return false; }
/* 1092 */   public final void removeIf(@NotNull Function2 predicate) { Intrinsics.checkNotNullParameter(predicate, "predicate"); int $i$f$removeIf = 0; ObjectLongMap this_$iv = this; int $i$f$forEachIndexed = 0; long[] m$iv = this_$iv.metadata;
...
/* 1139 */       remove((K)key);  } @PublishedApi public final void removeValueAt(int index) { this._size--; MutableObjectLongMap mutableObjectLongMap = this; long value$iv = 254L; int $i$f$writeMetadata = 0; long[] m$iv = mutableObjectLongMap.metadata;
...
/*      */     
/* 1147 */     int c$iv = mutableObjectLongMap._capacity;
...
/* 1177 */       metadata$iv[index >> 3] >> (index & 0x7) << 3 & 0xFFL) == 128L)) ? 1 : 0; MutableObjectLongMap mutableObjectLongMap = this; long value$iv = hash2; int $i$f$writeMetadata = 0;
/* 1178 */     long[] m$iv = mutableObjectLongMap.metadata;
...
/*      */     
/* 1186 */     int c$iv = mutableObjectLongMap._capacity;
...
/* 1196 */   private final void resizeStorage(int newCapacity) { long[] previousMetadata = this.metadata; Object[] previousKeys = this.keys; long[] previousValues = this.values; int previousCapacity = this._capacity; initializeStorage(newCapacity); Object[] newKeys = this.keys; long[] newValues = this.values; for (int i = 0; i < previousCapacity; i++) { int $i$f$isFull = 0; int $i$f$readRawMetadata = 0; if (((
...
/* 1201 */         int $i$f$h1 = 0, index = findFirstAvailableSlot(hash >>> 7); MutableObjectLongMap mutableObjectLongMap = this; int $i$f$h2 = 0;
/* 1202 */         long value$iv = (hash & 0x7F); int $i$f$writeMetadata = 0;
/* 1203 */         long[] m$iv = mutableObjectLongMap.metadata;
...
/*      */         
/* 1211 */         int c$iv = mutableObjectLongMap._capacity;
...
/* Location:              C:\Users\yuzum\Downloads\ej-release-243.132.63.zip!\ej\lib\collection-jvm-1.4.0.jar!\androidx\collection\MutableObjectLongMap.class
