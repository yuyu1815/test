...
/*     */   public static void onSelectFirstItem(@NotNull SelectableColumnOnKeyEvent $this, @NotNull List<SelectableLazyListKey> allKeys, @NotNull SelectableLazyListState state) {
...
/*     */   } public static void onScrollPageDownAndSelectItem(@NotNull SelectableColumnOnKeyEvent $this, @NotNull List<SelectableLazyListKey> keys, @NotNull SelectableLazyListState state) { Intrinsics.checkNotNullParameter(keys, "keys"); Intrinsics.checkNotNullParameter(state, "state"); int visibleSize = state.getLayoutInfo().getVisibleItemsInfo().size();
/*     */     state.getLastActiveItemIndex();
/*     */     int i = ((state.getLastActiveItemIndex() != null) ? state.getLastActiveItemIndex().intValue() : 0) + visibleSize, j = CollectionsKt.getLastIndex(keys), targetIndex = Math.min(i, j);
/*     */     state.setSelectedKeys(SetsKt.setOf(((SelectableLazyListKey)keys.get(targetIndex)).getKey()));
/* 131 */     state.setLastActiveItemIndex(Integer.valueOf(targetIndex)); } public static void onScrollPageUpAndExtendSelection(@NotNull SelectableColumnOnKeyEvent $this, @NotNull List keys, @NotNull SelectableLazyListState state) { Intrinsics.checkNotNullParameter(keys, "keys"); Intrinsics.checkNotNullParameter(state, "state"); int visibleSize = state.getLayoutInfo().getVisibleItemsInfo().size();
...
/* Location:              C:\Users\yuzum\Downloads\ej-release-243.132.63.zip!\ej\lib\jewel-foundation-243-0.27.0.jar!\org\jetbrains\jewel\foundation\lazy\SelectableColumnOnKeyEvent$DefaultImpls.class
